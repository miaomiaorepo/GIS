...
let data = authentic_res_dot.features.sort((a1, a2) => {
  return +a1.properties.authentic_original_count < +a2.properties.authentic_original_count
    ? 1
    : +a1.properties.authentic_original_count > +a2.properties.authentic_original_count
    ? -1
    : 0
})
...
function restaurantNodeCreator(data: { properties: Record<string, any> }[], length: number) {
  table.innerHTML = ""
...
function handleSearch(e: { key: string; target: { value: string } }) {
  if (e.key === "Enter") {
    ...
    previouFilterFeartures === "nta"
      ? (filteredData = data.filter(d => d.properties["NTAName"] === e.target.value))
      : (filteredData = filteredData.filter(d => d.properties["NTAName"] === e.target.value))
...
function searchNeighborhood(neighborhoodName: string) {
  ...
  const feature = features.find(
    feature => feature.properties![NEIGHBORHOOD_PROPERTY].toLowerCase() === neighborhoodName.toLowerCase()
  )
